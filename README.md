...
项目当前附带一份原创中文历史语料，主题是中国历代帝王人物小传，并基于这批语料生成了第一版 SFT / DPO 跑通数据。数据仅用于学习和 demo 验证。
...
项目已内置一份小型历史文本语料到 `data/`。你也可以替换成自己的纯文本数据。先训练 tokenizer：
...
当前仓库提供一份基于历史语料生成的跑通版 SFT 数据：

```text
SFT_data/emperor_sft_messages_v1.jsonl
```
...
当前仓库提供一份基于历史语料生成的跑通版 DPO 数据：

```text
DPO_data/emperor_dpo_pairs_v1.jsonl
```
...
并提供两份用于跑通 LoRA-SFT / LoRA-DPO 的 jsonl 示例数据：
...
这批数据是为了验证 mini LLM 的完整工程链路，不追求真实产品级效果。你可以直接用它跑通 tokenizer、预训练、SFT、DPO 和推理流程，也可以替换成自己的数据。
